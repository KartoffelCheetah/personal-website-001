...
def getDateTimeOriginal(filePath) :
    ...
    return subprocess.check_output(['identify', '-format', '%[EXIF:DateTimeOriginal]', filePath], timeout=3).decode('utf8')
def getDimensions(filePath) :
    """Returns dimensions of image in a tuple."""
    x,y = subprocess.check_output(['identify', '-format', '%[w]x%[h]', filePath], timeout=3).decode('utf8').split('x')
    return int(x), int(y)
