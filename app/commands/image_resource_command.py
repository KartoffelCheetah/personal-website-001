...
from app.managers.image_resource_manager import get_thumbnail_conf_by_thid, THID_ROOT, get_image_resource_entity_from_fs, save_image_thumbnails_to_fs
...
def status():
	...
	fs_content = set(os.listdir(os.environ['FOLDER_UPLOAD_ROOT']))
	...
	for conf in get_thumbnail_conf_by_thid(THID_ROOT).thumbnails:
		th_content = set(os.listdir(conf.folder))
		missing_parents = th_content - fs_content
		missing_thumbnails = fs_content - th_content

		if missing_parents:
			current_app.logger.info(f"No image for thumbnails: ({conf.folder}) | {missing_parents}")
		
		else:
			current_app.logger.info(f"OK! All thumbnails have parents in {conf.folder}")
		
		if missing_thumbnails:
			current_app.logger.info(f"No thumbnail for images: ({conf.folder}) | {missing_thumbnails}")
		
		else:
			current_app.logger.info(f"OK! All images have thumbnails in {conf.folder}")
